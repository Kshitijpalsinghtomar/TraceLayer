...
function SystemConfig() {
  ...
  const fetchModels = useAction(api.aiConfig.fetchOpenRouterModels);
  ...
  const currentOrModel = settings?.["openrouter_model"] || "";
  const showOpenRouter = currentProvider === "openrouter" || (!currentProvider && aiConfig?.provider === "openrouter");

  // Load models when OpenRouter section becomes visible
  ...
  }, [fetchModels, modelsLoaded, loadingModels]);

  useEffect(() => {
    if (showOpenRouter && !modelsLoaded) {
      loadModels();
    }
  }, [showOpenRouter, modelsLoaded, loadModels]);
  ...
  const handleSelectModel = async (modelId: string) => {
    await setSetting({ key: "openrouter_model", value: modelId });
    setDropdownOpen(false);
    setModelSearch("");
    setModelSaved(true);
  ...
  const handleProviderChange = async (value: string) => {
    await setSetting({ key: "default_provider", value });
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-8 max-w-[800px]">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-card rounded-2xl border border-border/50 p-6">
          ...
          {/* Live config status */}
          <div className={`mb-5 px-4 py-3 rounded-xl flex items-center gap-3 text-[13px] ${aiConfig?.configured
            ? "bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800"
            : "bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800"
            }`}>
            <div className={`w-2.5 h-2.5 rounded-full ${aiConfig?.configured ? "bg-emerald-500" : "bg-amber-500"}`} />
            {aiConfig?.configured ? (
              <span className="text-emerald-700 dark:text-emerald-300">
                Active: <strong>{aiConfig.provider}</strong> — {aiConfig.model}
              </span>
            ) : (
              <span className="text-amber-700 dark:text-amber-300">
                No AI configured — set a provider and ensure an API key is saved above
              </span>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <p className="text-[12px] text-muted-foreground">Used for all AI features (chat, pipeline, copilot)</p>
              </div>
              <select
                value={currentProvider || ""}
                onChange={(e) => handleProviderChange(e.target.value)}
                className="px-3 py-2 rounded-lg border border-border/60 text-[13px] bg-background min-w-[200px]"
              >
            ...
            {showOpenRouter && (
              <div className="py-3 pl-4 border-l-2 border-primary/20">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-[14px] font-medium">OpenRouter Model</p>
                    <p className="text-[12px] text-muted-foreground">
                      {modelsLoaded ? `${models.length} models available (${freeCount} free)` : "Loading models..."}
                    </p>
                  </div>
                  {modelSaved && (
                    <span className="text-[11px] px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 font-medium flex items-center gap-1 animate-in fade-in">
                ...
                {currentOrModel && (
                  <div className="mb-3 px-3 py-2.5 rounded-xl bg-primary/5 border border-primary/15 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-primary" />
                      <span className="text-[13px] font-medium">{currentOrModel}</span>
                    </div>
                    <span className="text-[10px] text-primary font-medium uppercase tracking-wider">Active</span>
                  </div>
                )}
                ...
                <div ref={dropdownRef} className="relative">
                  <div
                    className="flex items-center gap-2 px-3 py-2.5 rounded-xl border border-border/60 bg-background cursor-pointer hover:border-primary/40 transition-colors"
                  ...
                  >
                    ...
                    />
                    <label className="flex items-center gap-1.5 text-[11px] font-medium cursor-pointer select-none whitespace-nowrap px-2 py-1 rounded-lg hover:bg-muted/50 transition-colors" onClick={(e) => e.stopPropagation()}>
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-emerald-600">Free only</span>
                    </label>
                  ...
                  {/* Dropdown list */}
                  {dropdownOpen && (
                    <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-card border border-border rounded-xl shadow-xl max-h-[340px] overflow-y-auto overscroll-contain">
                      {loadingModels ? (
                        ...
                        <div className="py-6 text-center text-[13px] text-muted-foreground">
                          ...
                          {showFreeOnly && <span className="block text-[11px] mt-1">Try disabling "Free only" filter</span>}
                        </div>
                      ) : (
                        filteredModels.map((m) => (
                          <button
                            ...
                            className={`w-full text-left px-3.5 py-2.5 hover:bg-muted/60 transition-colors border-b border-border/30 last:border-0 ${currentOrModel === m.id ? "bg-primary/5" : ""
                              }`}
                          >
                            ...
                            <div className="flex items-center gap-3 mt-0.5">
                              <span className="text-[11px] text-muted-foreground truncate">{m.id}</span>
                              <span className="text-[10px] text-muted-foreground/60 shrink-0">
                                {m.contextLength > 0 ? `${Math.round(m.contextLength / 1024)}k ctx` : ""}
                              </span>
                              {!m.isFree && (
                                <span className="text-[10px] text-muted-foreground/60 shrink-0">
                                  {m.promptPricing}
